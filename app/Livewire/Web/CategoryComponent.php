...
class CategoryComponent extends Component
{
    ...
    public function mount($slug,$subCategorySlug = null)
    {
        $this->cat_slug = $slug;
        $this->sub_slug = $subCategorySlug;
        $this->minprice = 10;
        $this->maxprice = 10000;
    ...
    public function render()
    {
        try {
            ...
            $query = Service::where('category_id', $category->id)->where('status', 'active')
            ->where('approved', 'yes');
           if($this->sub_slug){
            $sub = SubCategory::where('slug',$this->sub_slug)->first();
        ...
        $services = $query->paginate($this->perPage);
            ...
            return view('livewire.web.category-component', ['services' => $services, 'categories' => $categories, 'types' => $types]);
        } catch (\Exception $e) {
