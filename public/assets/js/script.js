...
(function ($) {
	...
	$('.fav-icon').on('click', function () {
		...
		//$(this).children().toggleClass("bi bi-heart bi bi-heart");
	});
	...
	if ($('#data-table').length > 0) {
		$('#data-table').DataTable({
			"language": {
				...
				paginate: {
					next: 'Next <i class="bi bi-arrow-right-circle-fill ms-2"></i>',
					previous: '<i class="bi bi-arrow-left-circle-fill me-2"></i> Previous'
	...
	if ($('.owl-carousel.categories-slider-seven').length > 0) {
		$('.owl-carousel.categories-slider-seven').owlCarousel({
			...
			dots: true,
			smartSpeed: 2000,
			navText: ["<i class='bi bi-arrow-let-circle-fill'></i>", "<i class='bi bi-arrow-right-circle-fill'></i>"],
			navContainer: '.mynav',
			responsive: {
				0: {
					items: 1
	...
	if ($('.datetimepicker').length > 0) {
		$('.datetimepicker').datetimepicker({
			...
			icons: {
				...
				previous: 'bi bi-arrow-right-circle-fill'
			}
		});
	}
	...
	if ($('.toggle-password').length > 0) {
		$(document).on('click', '.toggle-password', function () {
			$(this).toggleClass("bi bi-eye bi bi-eye-off");
	...
	$(".add-extra").on('click', function () {
		...
		var servicecontent = '<div class="row service-cont">' +
			...
			'<a href="#" class="btn btn-danger-outline trash"><i class="fas fa-trash-alt"></i></a>' +
			'</div>' +
			'</div>' +
			'</div>' +
			'</div>';
	...
	$(".add-hours").on('click', function () {
		...
		var hourscontent = '<div class="row hours-cont">' +
			...
			'<span class="cus-icon"><i class="bi bi-clock"></i></span>' +
			'</div>' +
			'</div>' +
			'</div>' +
			'<div class="col-md-4">' +
	...
	if ($('#more').length > 0) {
		...
		function showMoreLess() {
			if (isLess) {
				...
				button.innerHTML = "Show less <i class='bi bi-arrow-up-circle ms-1'>";
			} else {
				isLess = true;
				container.style.height = '180px';
				button.innerHTML = "Show more <i class='bi bi-arrow-down-circle ms-1'></i>";
			}
		}
